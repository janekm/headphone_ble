...
#define MIN_CONN_INTERVAL               (SECOND_1_25_MS_UNITS / 50)                  /**< Minimum acceptable connection interval (0.5 seconds), Connection interval uses 1.25 ms units. */
#define MAX_CONN_INTERVAL               (SECOND_1_25_MS_UNITS / 20)                      /**< Maximum acceptable connection interval (1 second), Connection interval uses 1.25 ms units. */
...
ble_gatts_char_handles_t     accel_handles;
...
static void services_init(void)
{
    ...
    attr_char_value.init_len     = PACKET_LEN;
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = PACKET_LEN;
    attr_char_value.p_value      = packet;
...
bool accel_notification_enabled = false;

static void on_accel_cccd_write(ble_gatts_evt_write_t * p_evt_write)
{
    if (p_evt_write->len == 2)
    {
        if (ble_srv_is_notification_enabled(p_evt_write->data))
        {
            accel_notification_enabled = true;
        }
        else
        {
...
static void accel_on_write(ble_evt_t * p_ble_evt)
{
    ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
    
    if (p_evt_write->handle == accel_handles.cccd_handle)
    {
        on_accel_cccd_write(p_evt_write);
    }
}

void ble_accel_on_ble_evt(ble_evt_t * p_ble_evt) 
{
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GATTS_EVT_WRITE:
            accel_on_write(p_ble_evt);
            break;
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    ble_accel_on_ble_evt(p_ble_evt);
}
...
void ble_on_radio_active_evt(bool radio_active)
{
    ...
    uint8_t mma_read[] = {0x01};
    //ble_flash_on_radio_active_evt(radio_active);
    if ((radio_active) && accel_notification_enabled) {
        //for (int i = 0; i < 10000; i++) {
        //    __nop();
        //}
		//twi_master_transfer(tmp102_address | TWI_READ_BIT, (uint8_t*)&temperature, 2, TWI_ISSUE_STOP);
		twi_master_transfer(MMA8451_address, (uint8_t*)&mma_read, 1, TWI_DONT_ISSUE_STOP);
		twi_master_transfer(MMA8451_address | TWI_READ_BIT, (uint8_t*)&(packet[6]), 6, TWI_ISSUE_STOP);
        
        hvx_len = PACKET_LEN;

        memset(&hvx_params, 0, sizeof(hvx_params));
        
        hvx_params.handle   = accel_handles.value_handle;
        ...
        if ((err_code == NRF_SUCCESS) && (hvx_len != PACKET_LEN))
        {
...
int main(void)
{
    uint8_t mma_init_command[2] = { 0x2a, 0x2d};
	...
	twi_master_transfer(MMA8451_address, (uint8_t*)&mma_init_command, 2, TWI_ISSUE_STOP);
